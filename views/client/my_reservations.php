    ...
    <div class="max-w-6xl mx-auto mt-10 px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white shadow-md rounded-xl p-6 hover:shadow-xl transition border-l-4 
                <?php 
                if ($reservation->reservationStatus == 'confirmed') echo 'border-green-500';
                ...
                <div class="flex items-center justify-between mb-4">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium 
                        <?php 
                        if ($reservation->reservationStatus == 'confirmed') echo 'bg-green-100 text-green-800';
                        elseif ($reservation->reservationStatus == 'pending') echo 'bg-yellow-100 text-yellow-800';
                        else echo 'bg-gray-100 text-gray-800';
                        ?>">
                        <i class="fas fa-<?php 
                        if ($reservation->reservationStatus == 'confirmed') echo 'check-circle';
                        elseif ($reservation->reservationStatus == 'pending') echo 'clock';
                        else echo 'ban';
                        ?> mr-1"></i>
                        <?= ucfirst(htmlspecialchars($reservation->reservationStatus)) ?>
                    </span>
                </div>
